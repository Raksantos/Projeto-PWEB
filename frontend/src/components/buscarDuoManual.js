...
export default class BuscarDuoManual extends Component {
    ...
    handleSubmit(event) {
        this.setState({ duos: [] });
        axios.put('http://localhost:8000/busca/buscaManual', this.state, { headers: { 'token': this.state.usuario.token } })
            .then(res => {
                if (res.data.erro != 1) {
                    ...
                    this.setState({ duos: [], encontrado: true });
                }
    ...
    render() {
        if (this.state.selecionado === false) {
            ...
            return (<div>
                ...
                <ResultadoBusca duos={this.state.duos} />
            </div>)
        }
    }
}
